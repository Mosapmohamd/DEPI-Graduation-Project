...
model = tf.keras.models.load_model("Artifacts/gru_model.h5")
...
# Minimal slang dictionary (extend if needed)
slang_dict = {
    "$" : " dollar ",
    "€" : " euro ",
    "4ao" : "for adults only",
...
def preprocess_text(text):
    ...
    for word in words:
        ...
        if clean in slang_dict:
            processed_words.append(slang_dict[clean])
        else:
...
llm = ChatGoogleGenerativeAI(
    ...
    google_api_key='AIzaSyAU0YcbrnhcZdS3pWEZXBYVp0UQGUSgn0s' ,#st.secrets["GOOGLE_API_KEY"],
    temperature=0.2,  # Lower temperature for more practical recommendations
...
if st.button("Predict Sentiment"):
    if review:
        ...
        if sentiment[0] == 'positive':
          st.success(f"**Predicted Sentiment:** {sentiment[0].capitalize()}")
        else:
          st.error(f"**Predicted Sentiment:** {sentiment[0].capitalize()}")
